...
class Mysqldb {
	...
	get(table_name, field='*', where, callback){
		...
		this.connection.query('SELECT '+field+' FROM '+table_name+(where?' where '+where:''), callback);
		this.releaseCon();
	}
	select(table_name, fields='*', where='', limit='', callback){
		...
		this.connection.query('SELECT '+fields+' FROM '+table_name+(where?' where '+where:'')+limit, callback);
		this.releaseCon();
	}
	insert(table_name, data_ins, feilds, callback){
		if(table_name && data_ins && (!data_ins.length||(data_ins.length && data_ins.length!==0))){
			...
			if(key_str && val_str){
				this.connection.query('INSERT INTO '+table_name+' ('+key_str+') VALUES ('+val_str+')', callback);
				this.releaseCon();
			}else{
				return false;
			}
	...
	update(table_name, data_update, feilds, where, callback){
		if(table_name && data_update && (!data_update.length||(data_update.length && data_update.length!==0))){
			...
			if(update_str){
				this.connection.query('UPDATE '+table_name+' SET '+update_str.slice(1)+(where?' where '+where:''), callback);
				this.releaseCon();
			}
		}else{
			return false;
	...
	delete(table_name, where = 1, callback){
		...
		if(table_name){
			this.connection.query('DELETE FROM '+table_name+' WHERE '+where, callback);
		}
	}
	releaseCon(){
		// this.connection.end();
