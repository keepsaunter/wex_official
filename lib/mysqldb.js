	...
	query(str, callback){
		...
		if(this.connection) {
			this.connection.query(str, function(err, res, fields) {
				...
				if(this.time_releasecon) {
					clearTimeout(this.time_releasecon);
				}
				this.time_releasecon = setTimeout(function(){_self.releaseCon();}, 1000);
			});
		}else{
			callback('connection has been released;');
		}
	}
	static getDatetime() {     
